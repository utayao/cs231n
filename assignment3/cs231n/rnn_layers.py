...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  for t in xrange(x.shape[1]):
     xt = x[:,t,:]
     if t == 0:
        next_h, _ = rnn_step_forward(xt, h0, Wx, Wh, b)
        h = np.expand_dims(next_h, axis=1)
     else:
        prev_h = next_h
        next_h, _ = rnn_step_forward(xt, prev_h, Wx, Wh, b)
        next_h_new = np.expand_dims(next_h, axis=1)
        h = np.concatenate((h,next_h_new),axis=1)
...
def rnn_backward(dh, cache):
  ...
  for t in reversed(xrange(T)):
     ...
     if t == 0:
        ...
        h_prev_t = h[:,t-1,:]     
     cache_t = h_t, x_t, h_prev_t, Wx, Wh   
